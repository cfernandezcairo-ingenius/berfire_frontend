...
describe('ManufacturersAddEditComponent', () => {
  ...
  it('should call the edit method of ManufacturersService on new record', () => {
    component.id = 0;
    component.fg.setValue({ name: 'Test Bank', description: 'description 1', isActive: false });
    mockManufacturersService.add.and.returnValue(of({ success: true }));
    const payload = {  name: 'Test Bank', description: 'description 1', isActive: false };
    component.onSubmit(payload);
    expect(mockManufacturersService.add).toHaveBeenCalledWith({
      name: 'Test Bank', description: 'description 1', isActive: false
    });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });

  it('should call the edit method of ManufacturersService on editing a record', () => {
    component.id = 1;
    component.fg.setValue({  name: 'Test Bank', description: 'description 1', isActive: false });
    mockManufacturersService.edit.and.returnValue(of({ success: true }));
    component.onSubmit(component.model);
    expect(mockManufacturersService.edit).toHaveBeenCalledWith({
      id: 1,  name: 'Test Bank', description: 'description 1', isActive: false });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });
