...
describe('UnsubscribeReasonsListComponent', () => {
  ...
  beforeEach(async () => {
    unsubscribeSrv = jasmine.createSpyObj('UnsubscribeReasonsService', ['getAll', 'getByFields']);
    ...
    await TestBed.configureTestingModule({
      ...
      declarations: [],
      providers: [
        { provide: UnsubscribeReasonsService, useValue: unsubscribeSrv },
  ...
  it('should load all documents templates', () => {
    ...
    unsubscribeSrv.getAll.and.returnValue(of(mockData));

    component.loadAll();

    expect(unsubscribeSrv.getAll).toHaveBeenCalled();
  ...
  it('should handle errors when loading', () => {
    unsubscribeSrv.getAll.and.returnValue(of({ data: [] }));
    component.loadAll();
    expect(matSnackBar.open).toHaveBeenCalledWith('The data returned empty.', 'Close', Object({ duration: 2000 }));
  });
  ...
  it('should handle empty data when searching', () => {
    ...
    const mockData = { data: [] };
    unsubscribeSrv.getByFields.and.returnValue(of(mockData));
  ...
  it('should navigate to edit page', () => {
    ...
    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith(`/unsubscribe-reasons/edit/${JSON.stringify(row)}`);
  });

  it('should navigate to add page', () => {
    component.addItem();

    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith('/unsubscribe-reasons/edit/' + JSON.stringify({ id: 0 }));
  });
  ...
  it('should navigate to delete page', () => {
    ...
    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith(`/unsubscribe-reasons/delete/${JSON.stringify(id)}`);
  });
  ...
  it('should search data correctly', () => {
    ...
    expect(unsubscribeSrv.getByFields).toHaveBeenCalled();
    expect(component.dataSource.data.length).toBe(1);
    expect(component.loading).toBeFalse();
  });
});
