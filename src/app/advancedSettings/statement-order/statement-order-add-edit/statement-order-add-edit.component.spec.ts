...
const mockStatementOrderService = {
...
describe('StatementOrdersAddEditComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: StatementOrderService, useValue: mockStatementOrderService },
  ...
  it('should call the edit method of StatementOrderService on new record', () => {
    component.id = 0;
    component.fg.setValue({ name: 'Test Bank', description: 'desc 1', finalized: false });
    mockStatementOrderService.add.and.returnValue(of({ success: true }));
    const payload = { name: 'Test Bank', description: 'desc 1', finalized: false };
    component.onSubmit(payload);
    expect(mockStatementOrderService.add).toHaveBeenCalledWith({
      name: 'Test Bank', description: 'desc 1', finalized: false
    });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });

  it('should call the edit method of StatementOrderService on editing a record', () => {
    component.id = 1;
    component.fg.setValue({ name: 'Test Bank', description: 'desc 1', finalized: false });
    mockStatementOrderService.edit.and.returnValue(of({ success: true }));
    component.onSubmit(component.model);
    expect(mockStatementOrderService.edit).toHaveBeenCalledWith({
      id: 1,  name: 'Test Bank', description: 'desc 1', finalized: false });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });
