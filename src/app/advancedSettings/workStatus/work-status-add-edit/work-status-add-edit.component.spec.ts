...
describe('WorkStatusAddEditComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: WorkStatusService, useValue: mockWorkStatusService },
  ...
  it('should call the edit method of WorkStatusService on new record', () => {
    component.id = 0;
    component.fg.setValue({ name: 'Test Bank', description: 'description 1' });
    mockWorkStatusService.add.and.returnValue(of({ success: true }));
    const payload = { name: 'Test Bank', description: 'description 1' };
    component.onSubmit(payload);
    expect(mockWorkStatusService.add).toHaveBeenCalledWith({
      name: 'Test Bank', description: 'description 1'
    });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });

  it('should call the edit method of WorkStatusService on editing a record', () => {
    component.id = 1;
    component.fg.setValue({ name: 'Test Bank', description: 'description 1' });
    mockWorkStatusService.edit.and.returnValue(of({ success: true }));
    component.onSubmit(component.model);
    expect(mockWorkStatusService.edit).toHaveBeenCalledWith({
