...
describe('ContractsTypesListComponent', () => {
  ...
  let contractsTypesSrv: jasmine.SpyObj<ContractsTypesService>;
  ...
  beforeEach(async () => {
    contractsTypesSrv = jasmine.createSpyObj('ContractsTypesService', ['getAll', 'getByFields']);
    ...
    await TestBed.configureTestingModule({
      ...
      declarations: [],
      providers: [
        { provide: ContractsTypesService, useValue: contractsTypesSrv },
  ...
  it('should load all documents templates', () => {
    ...
    contractsTypesSrv.getAll.and.returnValue(of(mockData));

    component.loadAll();

    expect(contractsTypesSrv.getAll).toHaveBeenCalled();
  ...
  it('should handle errors when loading', () => {
    contractsTypesSrv.getAll.and.returnValue(of({ data: [] }));
    component.loadAll();
    expect(matSnackBar.open).toHaveBeenCalledWith('The data returned empty.', 'Close', Object({ duration: 2000 }));
  });
  ...
  it('should navigate to edit page', () => {
    ...
    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith(`/contracts-types/edit/${JSON.stringify(row)}`);
  });

  it('should navigate to add page', () => {
    component.addItem();

    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith('/contracts-types/edit/' + JSON.stringify({ id: 0 }));
  });
  ...
  it('should navigate to delete page', () => {
    ...
    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith(`/contracts-types/delete/${JSON.stringify(id)}`);
  });
  ...
  it('should search data correctly', () => {
    ...
    expect(contractsTypesSrv.getByFields).toHaveBeenCalled();
    expect(component.dataSource.data.length).toBe(1);
    expect(component.loading).toBeFalse();
  });
});
