...
describe('BillStatusAddEditComponent', () => {
  ...
  it('should call the edit method of BillStatusService on new record', () => {
    component.id = 0;
    component.fg.setValue({ name: 'Test Bank', isPaid: false, isReturned: false, isPending: false, isSent: false, isUnPaid: false });
    mockBillStatusService.add.and.returnValue(of({ success: true }));
    const payload = { name: 'Test Bank', isPaid: false, isReturned: false, isPending: false, isSent: false, isUnPaid: false };
    component.onSubmit(payload);
    expect(mockBillStatusService.add).toHaveBeenCalledWith({
      name: 'Test Bank', isPaid: false, isReturned: false, isPending: false, isSent: false, isUnPaid: false
    });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });

  it('should call the edit method of BillStatusService on editing a record', () => {
    component.id = 1;
    component.fg.setValue({ name: 'Test Bank', isPaid: false, isReturned: false, isPending: false, isSent: false, isUnPaid: false });
    mockBillStatusService.edit.and.returnValue(of({ success: true }));
    component.onSubmit(component.model);
    expect(mockBillStatusService.edit).toHaveBeenCalledWith({
      id: 1, name: 'Test Bank', isPaid: false, isReturned: false, isPending: false, isSent: false, isUnPaid: false });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });
