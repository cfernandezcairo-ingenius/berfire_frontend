...
const mockContractsTypesService = {
...
describe('ContractsTypesAddEditComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: ContractsTypesService, useValue: mockContractsTypesService },
  ...
  it('should call the edit method of ContractsTypesService on new record', () => {
    component.id = 0;
    component.fg.setValue({ name: 'Test Bank', duration: 1, isWarning: false });
    mockContractsTypesService.add.and.returnValue(of({ success: true }));
    const payload = { name: 'Test Bank', duration: 1, isWarning: false };
    component.onSubmit(payload);
    expect(mockContractsTypesService.add).toHaveBeenCalledWith({
      name: 'Test Bank', duration: 1, isWarning: false
    });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });

  it('should call the edit method of ContractsTypesService on editing a record', () => {
    component.id = 1;
    component.fg.setValue({name: 'Test Bank', duration: 1, isWarning: false });
    mockContractsTypesService.edit.and.returnValue(of({ success: true }));
    component.onSubmit(component.model);
    expect(mockContractsTypesService.edit).toHaveBeenCalledWith({
      id: 1, name: 'Test Bank', duration: 1, isWarning: false });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });
