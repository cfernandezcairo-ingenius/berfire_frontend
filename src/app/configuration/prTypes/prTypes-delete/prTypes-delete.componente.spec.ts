...
describe('PrTypesDeleteComponent', () => {
  ...
  it('should handle delete service error gracefully', async () => {
    mockPrTypesService.delete.and.returnValue(throwError(() => new Error('Error deleting'))); // Simulate an error
  ...
  it('should call delete service and show snackbar on user confirmation', async () => {
    ...
    expect(mockPrTypesService.delete).toHaveBeenCalledWith(1);
  });
});
