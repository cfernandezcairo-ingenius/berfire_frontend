...
describe('TaxesListComponent', () => {
  ...
  let taxesSrv: jasmine.SpyObj<TaxesService>;
  ...
  beforeEach(async () => {
    taxesSrv = jasmine.createSpyObj('TaxesService', ['getAll', 'getByFields']);
    ...
    await TestBed.configureTestingModule({
      ...
      declarations: [],
      providers: [
        { provide: TaxesService, useValue: taxesSrv },
  ...
  it('should load all documents templates', () => {
    const mockData = { data: [{ id: 1, title: 'Template 1', value: 'country1',equivalentSurcharge: 'eq' ,isIGIC: false }] };
    taxesSrv.getAll.and.returnValue(of(mockData));

    component.loadAll();

    expect(taxesSrv.getAll).toHaveBeenCalled();
  ...
  it('should handle errors when loading', () => {
    taxesSrv.getAll.and.returnValue(of({ data: [] }));
    component.loadAll();
    expect(matSnackBar.open).toHaveBeenCalledWith('The data returned empty.', 'Close', Object({ duration: 2000 }));
  });

  it('should handle empty data when searching', () => {
    const searchEvent = {  id: 1, title: 'Template 1', value: 'country1',equivalentSurcharge: 'eq' ,isIGIC: false };
    const mockData = { data: [] };
    taxesSrv.getByFields.and.returnValue(of(mockData));
  ...
  it('should navigate to edit page', () => {
    ...
    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith(`/taxes/edit/${JSON.stringify(row)}`);
  });

  it('should navigate to add page', () => {
    component.addItem();

    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith('/taxes/edit/' + JSON.stringify({ id: 0 }));
  });
  ...
  it('should navigate to delete page', () => {
    ...
    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith(`/taxes/delete/${JSON.stringify(id)}`);
  });

  it('should search data correctly', () => {
    const searchEvent = { data: {  id: 1, title: 'Template 1', value: 'country1',equivalentSurcharge: 'eq' ,isIGIC: false } };
    const mockData = { data: [searchEvent] };
    taxesSrv.getByFields.and.returnValue(of(mockData));

    component.searchData(searchEvent.data);

    expect(taxesSrv.getByFields).toHaveBeenCalled();
    expect(component.dataSource.data.length).toBe(1);
    expect(component.loading).toBeFalse();
  });
});
