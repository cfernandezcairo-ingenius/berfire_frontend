...
describe('RequestStatusDeleteComponent', () => {
  ...
  it('should handle delete service error gracefully', async () => {
    mockRequestStatusService.delete.and.returnValue(throwError(() => new Error('Error deleting'))); // Simulate an error
  ...
  it('should call delete service and show snackbar on user confirmation', async () => {
    ...
    expect(mockRequestStatusService.delete).toHaveBeenCalledWith(1);
  });
});
