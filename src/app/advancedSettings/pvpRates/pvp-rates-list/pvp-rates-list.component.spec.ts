...
describe('PvPRatesListComponent', () => {
  ...
  beforeEach(async () => {
    pvpRatesSrv = jasmine.createSpyObj('PVPRatesService', ['getAll', 'getByFields']);
    ...
    await TestBed.configureTestingModule({
      ...
      declarations: [],
      providers: [
        { provide: PVPRatesService, useValue: pvpRatesSrv },
  ...
  it('should load all documents templates', () => {
    ...
    pvpRatesSrv.getAll.and.returnValue(of(mockData));

    component.loadAll();

    expect(pvpRatesSrv.getAll).toHaveBeenCalled();
  ...
  it('should handle errors when loading', () => {
    pvpRatesSrv.getAll.and.returnValue(of({ data: [] }));
    component.loadAll();
    expect(matSnackBar.open).toHaveBeenCalledWith('The data returned empty.', 'Close', Object({ duration: 2000 }));
  });
  ...
  it('should handle empty data when searching', () => {
    ...
    const mockData = { data: [] };
    pvpRatesSrv.getByFields.and.returnValue(of(mockData));
  ...
  it('should navigate to edit page', () => {
    ...
    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith(`/pvp-rates/edit/${JSON.stringify(row)}`);
  });

  it('should navigate to add page', () => {
    component.addItem();

    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith('/pvp-rates/edit/' + JSON.stringify({ id: 0 }));
  });
  ...
  it('should navigate to delete page', () => {
    ...
    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith(`/pvp-rates/delete/${JSON.stringify(id)}`);
  });
  ...
  it('should search data correctly', () => {
    ...
    expect(pvpRatesSrv.getByFields).toHaveBeenCalled();
    expect(component.dataSource.data.length).toBe(1);
    expect(component.loading).toBeFalse();
  });
});
