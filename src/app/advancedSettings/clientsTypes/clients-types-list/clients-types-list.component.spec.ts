...
describe('ClientsTypesListComponent', () => {
  ...
  beforeEach(async () => {
    clientsTypesSrv = jasmine.createSpyObj('ClientsTypesService', ['getAll', 'getByFields']);
    ...
    await TestBed.configureTestingModule({
      ...
      declarations: [],
      providers: [
        { provide: ClientsTypesService, useValue: clientsTypesSrv },
  ...
  it('should load all documents templates', () => {
    const mockData = { data: [{ id: 1, name: 'Template 1', description: 'Type 1' }] };
    clientsTypesSrv.getAll.and.returnValue(of(mockData));

    component.loadAll();

    expect(clientsTypesSrv.getAll).toHaveBeenCalled();
  ...
  it('should handle errors when loading', () => {
    clientsTypesSrv.getAll.and.returnValue(of({ data: [] }));
    component.loadAll();
    expect(matSnackBar.open).toHaveBeenCalledWith('The data returned empty.', 'Close', Object({ duration: 2000 }));
  });
  ...
  it('should handle empty data when searching', () => {
    ...
    const mockData = { data: [] };
    clientsTypesSrv.getByFields.and.returnValue(of(mockData));
  ...
  it('should navigate to edit page', () => {
    ...
    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith(`/clients-types/edit/${JSON.stringify(row)}`);
  });

  it('should navigate to add page', () => {
    component.addItem();

    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith('/clients-types/edit/' + JSON.stringify({ id: 0 }));
  });
  ...
  it('should navigate to delete page', () => {
    ...
    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith(`/clients-types/delete/${JSON.stringify(id)}`);
  });
  ...
  it('should search data correctly', () => {
    ...
    expect(clientsTypesSrv.getByFields).toHaveBeenCalled();
    expect(component.dataSource.data.length).toBe(1);
    expect(component.loading).toBeFalse();
  });
});
