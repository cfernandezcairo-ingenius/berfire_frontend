...
describe('ClientsTypesAddEditComponent', () => {
  ...
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      providers: [
        ...
        { provide: ClientsTypesService, useValue: mockClientsTypesService },
  ...
  it('should call the edit method of ClientsTypesService on new record', () => {
    component.id = 0;
    component.fg.setValue({ name: 'Test Bank', description: 'Test' });
    mockClientsTypesService.add.and.returnValue(of({ success: true }));
    const payload = { name: 'Test Bank', description: 'Test' };
    component.onSubmit(payload);
    expect(mockClientsTypesService.add).toHaveBeenCalledWith({
      name: 'Test Bank', description: 'Test'
    });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });

  it('should call the edit method of ClientsTypesService on editing a record', () => {
    component.id = 1;
    component.fg.setValue({ name: 'Test Bank', description: 'Test' });
    mockClientsTypesService.edit.and.returnValue(of({ success: true }));
    component.onSubmit(component.model);
    expect(mockClientsTypesService.edit).toHaveBeenCalledWith({
      id: 1, name: 'Test Bank', description: 'Test' });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });
