...
describe('TaxesAddEditComponent', () => {
  ...
  // });
  it('should call the edit method of TaxesService on new record', () => {
    component.id = 0;
    component.fg.setValue({ title: 'Test Bank', value: '1',equivalentSurcharge: '2' ,isIGIC: false });
    mockTaxesService.add.and.returnValue(of({ success: true }));
    const payload = { title: 'Test Bank', value: '1',equivalentSurcharge: '2' ,isIGIC: false };
    component.onSubmit(payload);
    expect(mockTaxesService.add).toHaveBeenCalledWith({
      title: 'Test Bank', value: '1',equivalentSurcharge: '2' ,isIGIC: false
    });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });

  it('should call the edit method of TaxesService on editing a record', () => {
    component.id = 1;
    component.fg.setValue({title: 'Test Bank', value: '1',equivalentSurcharge: '2' ,isIGIC: false });
    mockTaxesService.edit.and.returnValue(of({ success: true }));
    component.onSubmit(component.model);
    expect(mockTaxesService.edit).toHaveBeenCalledWith({
      id: 1, title: 'Test Bank', value: '1',equivalentSurcharge: '2' ,isIGIC: false });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });
