...
describe('TechnicalsAddEditComponent', () => {
  ...
  it('should call the edit method of TechnicalsService on new record', () => {
    component.id = 0;
    component.fg.setValue({ name: 'Test Bank', firstSurname: 'first 1',secondSurName: 'second 1', user: 0 });
    mockTechnicalsService.add.and.returnValue(of({ success: true }));
    const payload = {  name: 'Test Bank', firstSurname: 'first 1',secondSurName: 'second 1', user: 0 };
    component.onSubmit(payload);
    expect(mockTechnicalsService.add).toHaveBeenCalledWith({
      name: 'Test Bank', firstSurname: 'first 1',secondSurName: 'second 1', user: 0
    });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });

  it('should call the edit method of TechnicalsService on editing a record', () => {
    component.id = 1;
    component.fg.setValue({ name: 'Test Bank', firstSurname: 'first 1',secondSurName: 'second 1', user: 0 });
    mockTechnicalsService.edit.and.returnValue(of({ success: true }));
    component.onSubmit(component.model);
    expect(mockTechnicalsService.edit).toHaveBeenCalledWith({
      id: 1, name: 'Test Bank', firstSurname: 'first 1',secondSurName: 'second 1', user: 0 });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });
