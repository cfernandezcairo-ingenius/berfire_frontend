...
describe('DeliveryNoteStatesAddEditComponent', () => {
  ...
  it('should call the edit method of DeliveryNoteStatesService on new record', () => {
    component.id = 0;
    component.fg.setValue({ name: 'Test Bank',confirmDeliveryNote: false });
    mockDeliveryNoteStatesService.add.and.returnValue(of({ success: true }));
    const payload = { name: 'Test Bank', confirmDeliveryNote: false };
    component.onSubmit(payload);
    expect(mockDeliveryNoteStatesService.add).toHaveBeenCalledWith({
      name: 'Test Bank', confirmDeliveryNote: false
    });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });

  it('should call the edit method of DeliveryNoteStatesService on editing a record', () => {
    component.id = 1;
    component.fg.setValue({name: 'Test Bank', confirmDeliveryNote: false });
    mockDeliveryNoteStatesService.edit.and.returnValue(of({ success: true }));
    component.onSubmit(component.model);
    expect(mockDeliveryNoteStatesService.edit).toHaveBeenCalledWith({
      id: 1, name: 'Test Bank', confirmDeliveryNote: false });
    expect(matSnackBarSpy.open).toHaveBeenCalled();
  });
