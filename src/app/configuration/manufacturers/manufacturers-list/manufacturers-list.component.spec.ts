...
describe('ManufacturersListComponent', () => {
  ...
  let manufacturersSrv: jasmine.SpyObj<ManufacturersService>;
  ...
  beforeEach(async () => {
    manufacturersSrv = jasmine.createSpyObj('ManufacturersService', ['getAll', 'getByFields']);
    ...
    await TestBed.configureTestingModule({
      ...
      declarations: [],
      providers: [
        { provide: ManufacturersService, useValue: manufacturersSrv },
  ...
  it('should load all documents templates', () => {
    const mockData = { data: [{ id: 1, name: 'Template 1', description: 'country1', isActive: false }] };
    manufacturersSrv.getAll.and.returnValue(of(mockData));

    component.loadAll();

    expect(manufacturersSrv.getAll).toHaveBeenCalled();
  ...
  it('should handle errors when loading', () => {
    manufacturersSrv.getAll.and.returnValue(of({ data: [] }));
    component.loadAll();
    expect(matSnackBar.open).toHaveBeenCalledWith('The data returned empty.', 'Close', Object({ duration: 2000 }));
  });

  it('should handle empty data when searching', () => {
    const searchEvent = {  id: 1, name: 'Template 1', description: 'country1', isActive:false };
    const mockData = { data: [] };
    manufacturersSrv.getByFields.and.returnValue(of(mockData));
  ...
  it('should navigate to edit page', () => {
    ...
    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith(`/manufacturers/edit/${JSON.stringify(row)}`);
  });

  it('should navigate to add page', () => {
    component.addItem();

    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith('/manufacturers/edit/' + JSON.stringify({ id: 0 }));
  });
  ...
  it('should navigate to delete page', () => {
    ...
    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith(`/manufacturers/delete/${JSON.stringify(id)}`);
  });

  it('should search data correctly', () => {
    const searchEvent = { data: { id: 1, name: 'Template 1', description: 'country1', isActive:false } };
    const mockData = { data: [searchEvent] };
    manufacturersSrv.getByFields.and.returnValue(of(mockData));

    component.searchData(searchEvent.data);

    expect(manufacturersSrv.getByFields).toHaveBeenCalled();
    expect(component.dataSource.data.length).toBe(1);
    expect(component.loading).toBeFalse();
  });
});
