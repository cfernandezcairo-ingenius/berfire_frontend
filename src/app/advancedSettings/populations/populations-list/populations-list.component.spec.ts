...
describe('PopulationsListComponent', () => {
  ...
  let populationsSrv: jasmine.SpyObj<PopulationsService>;
  ...
  beforeEach(async () => {
    populationsSrv = jasmine.createSpyObj('PopulationsService', ['getAll', 'getByFields']);
    ...
    await TestBed.configureTestingModule({
      ...
      declarations: [],
      providers: [
        { provide: PopulationsService, useValue: populationsSrv },
  ...
  it('should load all documents templates', () => {
    const mockData = { data: [{ id: 1, name: 'Template 1', country: 'country1', province: 'province1' ,active:false }] };
    populationsSrv.getAll.and.returnValue(of(mockData));

    component.loadAll();

    expect(populationsSrv.getAll).toHaveBeenCalled();
  ...
  it('should handle errors when loading', () => {
    populationsSrv.getAll.and.returnValue(of({ data: [] }));
    component.loadAll();
    expect(matSnackBar.open).toHaveBeenCalledWith('The data returned empty.', 'Close', Object({ duration: 2000 }));
  });

  it('should handle empty data when searching', () => {
    const searchEvent = {  id: 1, name: 'Template 1', country: 'country1', province: 'province1' ,active:false };
    const mockData = { data: [] };
    populationsSrv.getByFields.and.returnValue(of(mockData));
  ...
  it('should navigate to edit page', () => {
    ...
    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith(`/populations/edit/${JSON.stringify(row)}`);
  });

  it('should navigate to add page', () => {
    component.addItem();

    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith('/populations/edit/' + JSON.stringify({ id: 0 }));
  });
  ...
  it('should navigate to delete page', () => {
    ...
    expect(navigationSrv.NavigateTo).toHaveBeenCalledWith(`/populations/delete/${JSON.stringify(id)}`);
  });

  it('should search data correctly', () => {
    const searchEvent = { data: {  id: 1, name: 'Template 1', country: 'country1', province: 'province1' ,active:false } };
    const mockData = { data: [searchEvent] };
    populationsSrv.getByFields.and.returnValue(of(mockData));

    component.searchData(searchEvent.data);

    expect(populationsSrv.getByFields).toHaveBeenCalled();
    expect(component.dataSource.data.length).toBe(1);
    expect(component.loading).toBeFalse();
  });
});
